...
function pull_urls_div(){other_links_div.innerHTML="";$.ajax({ url:"pull.php", data:"pull_url=1", type:'post', dataType:'json', success:function(result){ other_links.innerHTML="";for(var i=0;i<result.length;i++){ other_links.innerHTML+=
	"<a name='other_link_href' target=_blank href="+result[i]['linkvalue']+ " onmouseover='pop_url();'"+
	//	" onmouseover='alert(\"hello\")'
	" style=\"font-size:"+Math.ceil(Math.random()*6+8)+"pt;\">"+result[i]['linkname']+"</a><a href='' onclick='del_url("+result[i]['linkid']+");return false;'>... </a>";} }});} 
</script>
<script type="text/javascript">
function del_url(id){ if(confirm("will delete url:"+id)) $.ajax({ url:"push.php",data:"link_id="+id+"&del_url=1", success:function(result){ pull_urls_div();return false; }});}
</script> 
<script type="text/javascript">
function push_urls(linkname,linkvalue){ $.ajax({ url:"push.php", data:"linkname="+linkname+"&linkvalue="+linkvalue+"&push_url=1", success:function(result){alert(result);}}); }
</script>
<script type="text/javascript">
function push_notes(title,content,link){ $.ajax({ url:"push.php", data:"title="+title+"&content="+content+"&link="+link+"&push_note=1", success:function(result){alert(result);}}); }
</script>
<script type="text/javascript">
...
td#col_0{ opacity:0.1; }
td#col_1{ opacity:0.5 }
td#col_2{ opacity:0.8 }
td#col_3{ opacity:0.8; }
td#col_4{ opacity:0.1; }
#container { 
...
#footer { border-top: none !important;  
  "background: url(blog/bot_bg.jpg) no-repeat bottom center; "
	...
	<div class="urls-popup" style="width: 250px; height: 100px; overflow: auto; padding: 1em; margin: 1em; font-size: 12px; display:none ; position=absolute;">
	<table name='urls-table' bgcolor='#eeeeff'>
	<tr><td><h3> this is a link name</h3></td><td><h4><a href="localhost">this is a link url</a></h4></td></tr>
	</table>
	</div>
	...
	<table width=82% style="border-collapse:collapse;margin-right:110px;table-layout:fixed;background-color:;"  align="center" border=0><tr>
	<td id='col_0'width='1%' border='hidden' bgcolor='fafafa' align='center'border=0></td>
	<td id='col_1' width=23% bgcolor='f1e6d4'><!-- bgcolor='F1E6D4' -->  <!--'F1E6D4'-->
	<h3>access</h3>
	<ul><li>
	...
	<a href="http://localhost/phpadmin" target="_blank">phpadmin</a>
	<a href="http://localhost/bbs" target="_blank">bbs</a>
	</li><li>
	1.music
	...
	<div name="youdao_dict">
	<form autocomplete="off" id="searchForm" action="http://dict.youdao.com/search?le=eng&keyfrom=dict.top" target="_blank">
	<input size='7px' type="text" class="inp1" name="q" value="">
	<input type="submit" value="search">有道词典</input>
	</form></div>
	<div name="smth_work">
	<form autocomplete="off" method='get' action='http://ar.newsmth.net/searchArticle.do?subject=&author=&boardname=&start=&end=&sbyt=true' target="_blank">
	<input size='7px' type="hidden" class="inp1" name="b" value="WorkLife">
	<input size='7px' type="hidden" class="inp1" name="au" value="">
	<input size='7px' type="text" class="inp1" name="body" value="">
	<input type="submit" value="search">水木职业</input>
	</form></div>
	</li><li>
	5.weibo
	<table frame="hsides" width=100% style="table-layout:fixed;" >
...
function pull_reply(pg){ $.ajax({url:"pull.php",dataType:"json",data:"reply_pg="+pg+"&pgsize=5&pull_reply=1",success:function(res){display_reply(res);} }); }
</script>
<?php
$begin=$pg-5>0?$pg-5:0; $end=$pg+5; for($k=$begin;$k<$end;$k++){ ?>
  <a href="javascript:void(0);" onclick="pull_reply(<?php echo $k ?>);" ><?php echo $k?></a>
	...
	<td id='col_3' width=53% bgcolor='#fafafa'> <!--efffff-->
	<div>
	<ul><li>
	在线评测平台
	<table frame="hsides" width=100% style="table-layout:fixed;" >
	...
	<div name="other_links_div" id="other_links_div"> </div>
	<table name="other_links" id="other_links" align='center'> </table>
	<script type="text/javascript">
	//	pull_urls();
	pull_urls_div();
	</script>
	...
	<td><a  target="_blank" href="http://localhost1/wjbzbmr/blog">CLJ</a></td>
	<td><a  target="_blank" href="http://localhost1/%D6%D0%B9%FA%C4%D4%BD%EE/blog">7k+顾昱洲(GYZ/ym)</a></td>
	<td><a  target="_blank" href="http://localhost1/oimaster/blog">oimaster(JZP)</a></td>
	<td><a  target="_blank" href="http://localhost1/coolinging/blog">coolinging</a></td>
	<td><a  target="_blank" href="http://hi.baidu.com/new/billdu">applepi杜宇飞(dyf)</a></td>
	<td><a  target="_blank" href="http://localhost1/mikeni2006/blog">nzk</a></td>
	<td><a  target="_blank" href="http://localhost1/coolypf/blog">coolypf</a></td>
	...
	<td><a  target="_blank" href="http://localhost1/lydrainbowcat/blog">lyd(李煜东)</a></td>
	...
	<td><a  target="_blank" href=http://blog.csdn.net/cqlf__/article/details/8864882/">acmer</a></td>
	</tr>
	</table>
	</li><li>
	基础库
	...
	<td><a  target="_blank" href="http://www.php.net/manual/zh/reserved.variables.session.php">php</a></td>
	...
	projects
	<table>
	<td><a  target="_blank" href="https://github.com/stars"><u>github</u></a></td>
	<td><a  target="_blank" href="http://i.pku.edu.cn/renrenplugin">renrenp</a></td>
	<td><a  target="_blank" href="http://astar.baidu.com/index.php?mod=cms&act=gy_detail&id=18">ypf</a></td>
	<td><a  target="_blank" href="http://ftp3.ie.freebsd.org/pub/">oschina</a></td>
	<td><a  target="_blank" href="http://www.csdn.net/article/2011-08-11/302961">crazy web</a></td>
	<td><a  target="_blank" href="https://code.google.com/p/mrrm/">ypf_mrrm...</a></td>
	</table>
	</li></ul>
	</div>
	</td>

	<td id='col_2' width=23% bgcolor='fafafa' border=0><!-- effeff-->
	<div>
	...
	<input size='14px' name="passwd" id="passwd" type="password" maxlength="30" tabindex="2"> 
	...
	<label><font size="2" p strong>链接名称</font></label><input size='15px' name="link_name" value="link name"  onclick="this.value=''"/><br/>
	<label><font size="2" p strong>链接内容</font></label><input size='15px' name="link_value" value=""  onclick="this.value=''"/><br/>
	</form>
	<button onclick="push_urls(push_url_form.link_name.value,push_url_form.link_value.value);">添加</button>
	<button type="ff" id="dd" name="dd" onclick="del_urls();return;">空</button>
	<script>s=true;function display_urls(){ if(s) {s=false;pull_urls();} else pull_urls_div();}</script>
	<button type="ff" id="dd" name="dd" onclick="display_urls();return;">显</button>
	</li>
	<li>
	速记
	<form id="push_note_form" name="push_note_form" method="post" action="push.php">
	<label><font size="2" p strong>笔记标题</font></label><input size='15px' name="note_title" value="link name"  onclick="this.value=''"/><br/>
	<label><font size="2" p strong>笔记内容</font></label><input size='15px' name="note_content" value=""  onclick="this.value=''"/><br/>
	<label><font size="2" p strong>笔记链接</font></label><input size='15px' name="note_link" value=""  onclick="this.value=''"/><br/>
	</form>
	<button onclick="push_notes(push_note_form.note_title.value,push_note_form.note_content.value,push_note_form.note_link.value);">添加</button>
	<a target="__blank" href="pull.php?pull_note=1">显示</a>
	</li>
	</ul>
	</div>
	...
	<td id='col_4'width='1%' border='hidden' bgcolor='fafafa' align='center'border=0>
	<strong>  </strong><br>
	...
	<div  style="Z-INDEX:-1;POSITION:absolute;WIDTH:0px;TOP:10px;LEFT:0px"> </div>
	<!-- <div id='footer'></div> -->
	</html>
